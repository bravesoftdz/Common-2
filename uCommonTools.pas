...
function icsExtractDigits(S: String): String;
function icsExpandString(S: String): String;
...
function icsGetSpecialFolderLocation(Wnd: HWND; nFolder: Integer): String;
...
function icsStartProcess(FileName: String; Params: String = ''; CurrentDir: String = ''; ShowWindow: Integer = SW_SHOW; WaitInputIdle: Boolean = False; WaitSingleObject: Boolean = False): Cardinal;
function icsStartProcessEx(const FileName, Params, CurrentDir: String; const ShowWindow: Integer; const WaitInputIdle, WaitSingleObject, RunAsAdmin: Boolean; var ExitCode: Cardinal): Boolean;
...
//function icsGetURLComponents(const AURL: String): TURLComponents;
...
procedure icsDoExitWindows(ExitWindowsAction: TExitWindowsAction);
...
function icsGetLocalIPs(IPs: TStringList): Integer;
...
function icsGetSpecialFolderLocation(Wnd: HWND; nFolder: Integer): String;
 ...
 var
   ...
   Buf: array[0..MAX_PATH - 1] of Char;
begin
  if (SHGetSpecialFolderLocation(Wnd, nFolder, PIDL) = NOERROR) and SHGetPathFromIDList(PIDL, Buf) then Result := Buf else Result := '';
end;
...
begin
  ...
  try
        ...
        if not INetFileTerminated and (hRequest <> nil) and HttpSendRequest(hRequest, nil, 0, nil, 0) then begin
          ...
          if Ok then begin
            ...
            if HttpQueryInfo(hRequest, HTTP_QUERY_CONTENT_LENGTH, @StringData, dwBufLen, dwIndex) then begin
              FileSize := StrToInt(String(StringData));
              if FileSize > 0 then begin
                if IsWindow(Wnd) then SendMessage(Wnd, PROG_MESSAGE_PROGRESS, Ord(paInit), FileSize);
                AssignFile(F, FName);
                Rewrite(F, 1);
                CurrentPos := 0;
                ...
                CloseFile(F);
                Result := (icsFileExistsEx(FName) and (icsGetFileSize(FName) = FileSize));
              end;
            end;
          end;
        end;
    ...
    InternetCloseHandle(hSession);
    if IsWindow(Wnd) then begin
      if Result then SendMessage(Wnd, PROG_MESSAGE_PROGRESS, Ord(paOk), 0) else SendMessage(Wnd, PROG_MESSAGE_PROGRESS, Ord(paFail), 0);
    end;
  end;
end;
...
begin
  ...
  if ExitWindowsAction in [ewaShutDown, ewaReboot, ewaLogoff] then begin
    ...
    if (Boolean(Flags) and IsPwrHibernateAllowed) or (not Boolean(Flags) and IsPwrSuspendAllowed) then SetSuspendState(Boolean(Flags), False, False);
  end;
end;
...
function icsGetLocalIPs(IPs: TStringList): Integer;
 ...
 var
   ...
   GInitData: TWSAData;
begin
  if not Assigned(IPs) then IPs := TStringList.Create else IPs.Clear;
